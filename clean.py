...
df = df.reset_index(drop=True)
new_df = pd.DataFrame(
    columns=["patient_id_junk", "type", "experiment", "intervention", "value"]
)
for i in range(1, 37):
    for j in range(1, 145):
        new_df = new_df.append(
            {
                "patient_id_junk": df[i][0],
                "type": df[0][j],
                "experiment": df[37][j],
                "intervention": df[38][j],
                "value": df[i][j],
            },
            ignore_index=True,
        )
split_df = pd.DataFrame(
    new_df.patient_id_junk.str.split(" ", 1).tolist(), columns=["patient_id", "level"]
)
